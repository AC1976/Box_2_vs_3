...
class Box2:
    ...
    def calculate_year(self, year, market_return_rate=0.05, dividend_yield=0.02,
                        coupon=0.035, rental_return=0.065, fee_amount=0, inflation=0.025, partners=True, mtm=True,
        ...
        row[11] = row[0] + row [3]
        row[12] = row[1]
        row[13] = row[2] * (1+inflation)

        # Column 14, 15, 16: Book value of investments
        row[14] = row[11] if mtm == True else self.initial_equity
...
def run_example():
    ...
    model = Box2(100000, 100000, 100000)
