...
class Box3:
    def __init__(self, initial_equity, initial_bonds, initial_property):
        self.results = []
    ...
    def calculate_year(self, year, market_return_rate=0.05, dividend_yield=0.02,
                        coupon=0.035, rental_return=0.065, inflation=0.025, partners=False, franchise=1250,
                        box3Rate=0.36):
        ...
        row[3] = row[0] * market_return_rate
        row[4] = row[0] * dividend_yield
        row[5] = row[1] * coupon
        row[6] = row[2] * rental_return
    
        # Column 7, 8: taxable income and current tax expense
        row[7] = row[3]+row[4]+row[5]+row[6]
        row[8] = (row[7]-2*franchise)*box3Rate if partners is True else (row[7]-franchise)*box3Rate
...
# Example usage showing year-over-year comparisons
def run_example():
    # Initialize model with $100,000 initial investment
    model = Box3(100000, 100000, 100000)
    
    # Run for 5 years to demonstrate year-over-year changes
    results = model.run_model(5)
    df = pd.DataFrame(results, columns=model.column_names).astype('int')
